...
function App() {
  ...
  const upcomingAppointment = appointments.find((appointment) => appointment.status === 'booked');
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      try {
        setError('');
        const [providerData, appointmentData] = await Promise.all([fetchProviders(), fetchAppointments()]);
  ...
  const navItems = [
    ...
    { id: 'appointments', label: 'My Appointments', icon: FaBell },
  ...
  const showProviders = !isProvider && (showDashboard || activeView === 'providers' || activeView === 'book');
  const showBooking = !isProvider && (showDashboard || activeView === 'book');
          ...
          {showBooking && <section className="section">
            ...
            <div className="booking-layout">
                ...
                <div className="calendar-grid">
                  {monthDays.map((date, idx) => {
                    ...
                    return (
                      <button
                        key={idx}
                        className={`calendar-day ${dayValue === selectedDate ? 'selected' : ''} ${!date ? 'empty' : ''}`}
                        onClick={() => date && setSelectedDate(dayValue)}
                      >
                        {date?.getDate()}
                      </button>
                    );
              ...
              <div className="timeslots-section">
                ...
                <button className="primary-button" disabled={!session || !selectedSlot || loading} onClick={handleBookAppointment}>
                  {loading ? 'Working...' : 'Book Appointment'}
                </button>
              </div>
            </div>
          </section>}
            ...
            <section className="section">
              <div className="section-header">
                <div>
                  ...
                  <p>Booked, cancelled, and completed visits from PostgreSQL.</p>
                </div>
              </div>
              <div className="appointment-list">
                {appointments.map((appointment) => (
                  <div className="appointment-row" key={appointment.id}>
                    <div>
                      <h3>{isProvider ? appointment.patient_name : appointment.provider_name}</h3>
                      <p>{isProvider ? appointment.reason || 'Patient visit' : appointment.specialty}</p>
